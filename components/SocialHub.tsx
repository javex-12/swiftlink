...
export function SocialHub({ storeId, accentColor, defaultTab = "feed", onBack }: SocialHubProps) {
  ...
  const [setupAvatar, setSetupAvatar] = useState("User"); // Identifier for icon
      ...
      <div className="h-full bg-white dark:bg-black p-8 flex flex-col items-center justify-center">
        <div className="w-full max-w-sm space-y-12">
          <div className="text-center space-y-4">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-28 h-28 rounded-[2.5rem] bg-slate-900 dark:bg-white flex items-center justify-center mx-auto shadow-2xl relative group">
              <AvatarIcon name={setupAvatar} />
              <button className="absolute -bottom-2 -right-2 w-10 h-10 rounded-full bg-emerald-500 border-4 border-white dark:border-black flex items-center justify-center text-white shadow-lg"><Settings size={16} /></button>
            </motion.div>
            <div className="space-y-1">
              <h2 className="text-3xl font-black tracking-tight dark:text-white italic uppercase">The Setup</h2>
              <p className="text-sm text-slate-400 font-medium">Claim your professional handle.</p>
            </div>
          </div>
          <div className="flex justify-center gap-3 flex-wrap">
            {PROF_AVATARS.map(a => (
               <button key={a.icon.name} onClick={() => setSetupAvatar(a.icon.name)} className={cn("w-12 h-12 rounded-2xl flex items-center justify-center transition-all overflow-hidden", setupAvatar === a.icon.name ? "ring-4 ring-emerald-500 scale-110" : "opacity-60")}>
                  <AvatarIcon name={a.icon.name} />
               </button>
            ))}
          </div>
          <div className="space-y-4">
            <input value={setupName} onChange={e => setSetupName(e.target.value)} placeholder="Display Name" className="w-full px-6 py-4 bg-slate-50 dark:bg-zinc-900 rounded-3xl border-none outline-none font-bold dark:text-white shadow-inner" />
            <div className="relative">
              <AtSign size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400" />
              <input value={setupUsername} onChange={e => setSetupUsername(e.target.value)} placeholder="username" className="w-full pl-12 pr-6 py-4 bg-slate-50 dark:bg-zinc-900 rounded-3xl border-none outline-none font-bold dark:text-white shadow-inner" />
            </div>
            <button onClick={handleProfileSetup} disabled={!setupUsername.trim()} className="w-full py-5 bg-slate-900 dark:bg-white text-white dark:text-black rounded-3xl font-black uppercase tracking-[0.2em] text-xs shadow-2xl transition-transform active:scale-95">Create Identity</button>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5 mt-0.5">
              ...
              Live · {state.bizName || "SwiftLink"}
            </p>
          </div>
        </div>
        <button onClick={() => setTab("profile")} className="w-10 h-10 rounded-2xl border-2 border-slate-100 dark:border-zinc-800 overflow-hidden shadow-sm active:scale-90 transition-transform">
           <AvatarIcon name={myProfile?.avatar_url} />
        </button>
      </header>
            ...
            <motion.div key="feed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="divide-y divide-slate-50 dark:divide-zinc-900">
              {loading ? (
                ...
                <div className="text-center py-32 px-10">
                   ...
                   <h2 className="text-xl font-black text-slate-900 dark:text-white italic uppercase">No Activity Yet</h2>
                   <button onClick={() => setTab("post")} className="mt-10 px-10 py-4 bg-slate-900 dark:bg-white text-white dark:text-black rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl transition-transform active:scale-95">First Post</button>
                </div>
              ) : (
                reviews.map((r, i) => (
                  <motion.article key={r.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="p-5 md:p-10 flex gap-5 hover:bg-slate-50/50 dark:hover:bg-zinc-900/30 transition-colors group cursor-pointer" onClick={() => { setActiveThread(r); fetchComments(r.id); }}>
                    <div className="shrink-0 w-14 h-14">
                      <AvatarIcon name={r.author_avatar} />
                    </div>
          ...
          {tab === "post" && (
            <motion.div key="post" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="p-6 md:p-12 max-w-2xl mx-auto w-full">
               <div className="flex gap-6">
                  <div className="w-16 h-16 shrink-0 shadow-xl">
                    <AvatarIcon name={myProfile?.avatar_url} />
                  </div>
          ...
          <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-0 z-[100] bg-white dark:bg-black flex flex-col">
             ...
             <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
                <div className="flex gap-4">
                   <div className="w-12 h-12 shrink-0"><AvatarIcon name={activeThread.author_avatar} /></div>
                   <div className="flex-1 min-w-0">
                   ...
                   {loadingComments ? <Loader2 className="animate-spin text-slate-300 mx-auto" size={24} /> : (
                     comments.map((c, i) => (
                       <div key={i} className="flex gap-4">
                          <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-zinc-900 flex items-center justify-center text-xl shrink-0"><MessageSquare size={18} /></div>
             ...
             <div className="p-4 bg-white/80 dark:bg-black/80 backdrop-blur-xl border-t border-slate-100 dark:border-zinc-900 pb-10">
                <div className="max-w-2xl mx-auto flex gap-3 items-end">
                   <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Post your reply..." className="flex-1 bg-slate-50 dark:bg-zinc-900 rounded-2xl px-5 py-4 text-sm outline-none resize-none min-h-[56px] dark:text-white shadow-inner" rows={1} />
