...
export function SocialHub({ storeId, accentColor, defaultTab = "feed", onBack }: SocialHubProps) {
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviews.filter(r => r.message.toLowerCase().includes(q) || r.author_name.toLowerCase().includes(q));
  }, [reviews, searchQuery]);
      ...
      <div className="h-full bg-white dark:bg-black p-8 flex flex-col items-center justify-center">
        <div className="w-full max-w-sm space-y-12">
          <div className="text-center space-y-4">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-28 h-28 rounded-[2.5rem] mx-auto shadow-2xl relative"><AvatarIcon src={setupAvatar} /><button className="absolute -bottom-2 -right-2 w-10 h-10 rounded-full bg-emerald-500 border-4 border-white dark:border-black flex items-center justify-center text-white shadow-lg"><Settings size={16} /></button></motion.div>
          ...
          <div className="space-y-4">
            ...
            <button onClick={async () => { if (!user || !setupUsername.trim()) return; const { error } = await supabase.from("social_profiles").upsert({ id: user.id, username: setupUsername.toLowerCase().trim(), display_name: setupName.trim() || setupUsername.trim(), avatar_url: setupAvatar }); if (error) { addToast("Username taken!", "error"); } else { setMyProfile({ id: user.id, username: setupUsername, display_name: setupName, avatar_url: setupAvatar }); setOnboarding(false); addToast("Profile Ready!", "success"); } }} disabled={!setupUsername.trim()} className="w-full py-5 bg-slate-900 dark:bg-white text-white dark:text-black rounded-3xl font-black uppercase tracking-widest text-xs shadow-2xl">Start Vibing</button>
          </div>
        </div>
      </div>
    );
        ...
        <AnimatePresence mode="wait">
          ...
          {tab === "report" && (
            <motion.div key="report" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 space-y-10">
               <div className="text-center space-y-3"><div className="w-20 h-20 bg-rose-500/10 rounded-[2rem] flex items-center justify-center text-rose-500 mx-auto border-2 border-rose-500/20 shadow-xl shadow-rose-500/10"><AlertTriangle size={32} /></div><div><h2 className="text-2xl font-black italic dark:text-white uppercase tracking-tighter">Support Hub</h2><p className="text-sm text-slate-400 font-medium tracking-tight">How can we improve your experience?</p></div></div>
               <div className="grid grid-cols-2 gap-4">{[{ id: 'bug', icon: Bug, label: 'Report Bug', color: 'bg-rose-500' }, { id: 'idea', icon: Lightbulb, label: 'Feature Idea', color: 'bg-amber-500' }, { id: 'vibe', icon: Sparkles, label: 'General Feedback', color: 'bg-emerald-500' }, { id: 'shield', icon: ShieldCheck, label: 'Safety Concern', color: 'bg-blue-500' }].map(item => (<button key={item.id} className="p-6 rounded-[2rem] bg-white dark:bg-zinc-900 border border-slate-100 dark:border-white/5 text-left space-y-4 hover:shadow-2xl transition-all group active:scale-95"><div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg", item.color)}><item.icon size={22} /></div><span className="block text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{item.label}</span></button>))}</div>
               <div className="bg-slate-50 dark:bg-zinc-950 p-6 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-zinc-800"><textarea placeholder="Write your message here..." className="w-full bg-transparent border-none outline-none text-slate-900 dark:text-white font-medium resize-none min-h-[120px] text-lg" /><button className="w-full mt-4 py-5 bg-slate-900 dark:bg-white text-white dark:text-black rounded-3xl font-black uppercase tracking-widest text-xs shadow-xl active:scale-95 transition-transform">Submit to HQ</button></div>
            </motion.div>
          )}

          {tab === "profile" && myProfile && (
            <motion.div key="profile" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 md:p-16 max-w-2xl mx-auto space-y-12">
               <div className="flex flex-col items-center text-center space-y-6"><div className="w-32 h-32 rounded-[3rem] mx-auto shadow-2xl relative"><AvatarIcon src={myProfile.avatar_url} /><button className="absolute -bottom-2 -right-2 w-10 h-10 rounded-full bg-emerald-500 border-4 border-white dark:border-black flex items-center justify-center text-white"><Settings size={18} /></button></div><div className="space-y-1"><h2 className="text-4xl font-black tracking-tighter dark:text-white italic uppercase">{myProfile.display_name}</h2><p className="text-lg text-slate-400 font-bold uppercase">@{myProfile.username}</p></div><div className="flex gap-10 py-6"><div className="text-center"><p className="text-2xl font-black dark:text-white">{stats.posts}</p><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Posts</p></div><div className="text-center"><p className="text-2xl font-black dark:text-white">{stats.followers}</p><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Followers</p></div><div className="text-center"><p className="text-2xl font-black dark:text-white">{stats.following}</p><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Following</p></div></div></div>
