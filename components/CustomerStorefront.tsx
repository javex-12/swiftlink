...
export function CustomerStorefront({
  ...
  // Use contrasting colors for header and navigation to fix visibility
  ...
  const headerTextColor = getContrastColor(bgColor);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center selection:bg-emerald-500 selection:text-white w-full overflow-x-hidden"
      ...
      <style>{`
         ...
         /* Fix for black on black text */
         .text-gray-900, .text-slate-900 { color: var(--text-color) !important; }
         .storefront-header .text-gray-900, .storefront-header .text-slate-900 { color: var(--header-text-color) !important; }
         .storefront-header svg { color: var(--header-text-color) !important; }
            ...
            >
                  ...
                  <div className="w-full px-4 md:px-12 py-3 md:py-5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button onClick={() => goTab("cart")} className="relative p-1.5 hover:bg-black/5 rounded-full transition-colors">
                        ...
                        <AnimatePresence>
                          {cartItemCount > 0 && (
                            <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="absolute -top-1 -right-1 w-4 h-4 bg-emerald-500 text-white text-[7px] font-black rounded-full flex items-center justify-center">
                              {cartItemCount}
                            </motion.span>
                          )}
                        </AnimatePresence>
                ...
                <div className="flex flex-col">
                  <div className="w-full">
                    {(() => {
                        const Hero = HeroTemplates[s.heroTemplateId || "hero-1"] || HeroTemplates["hero-1"];
                        return <Hero state={s} onShopClick={() => {
                  ...
                  <div className="w-full flex-1">
                    <div className="max-w-screen-xl mx-auto px-4 md:px-12 py-4 md:py-8">
                        ...
                        {(() => {
                            const Catalog = CatalogTemplates[s.catalogTemplateId || "catalog-1"] || CatalogTemplates["catalog-1"];
                            return <Catalog state={s} products={filteredProducts} onProductClick={(p: Product) => { setSelectedProduct(p); changeScreen("product"); logEvent("product_click", { productId: p.id }); }} />;
                        })()}
                        {(() => {
                            const About = AboutTemplates[s.aboutTemplateId || "about-1"] || AboutTemplates["about-1"];
                            return <About state={s} />;
                        })()}
                    </div>
                  </div>
                  <div className="w-full mt-auto">
                    {(() => {
                        const Footer = FooterTemplates[s.footerTemplateId || "footer-1"] || FooterTemplates["footer-1"];
                        return <Footer state={s} />;
                    })()}
                  </div>
                </div>
            </div>
                ...
                {screen !== "home" && (
                    ...
                    >
                        {screen === "product" && selectedProduct && (
                            <div className="w-full md:flex md:items-stretch md:min-h-screen" style={{ color: textColor }}>
                                ...
                                <div className="p-6 md:p-12 md:w-1/2 flex flex-col justify-center">
                                    <div className="flex items-start justify-between">
                                        <h1 className="text-2xl md:text-4xl font-black leading-tight">{selectedProduct.name}</h1>
                                        <p className="text-2xl md:text-4xl font-black text-emerald-600">{s.currency}{Number(selectedProduct.price).toLocaleString()}</p>
                                    </div>
                                    <p className="text-xs md:text-base opacity-60 mt-6 md:mt-10 leading-relaxed max-w-md">{selectedProduct.description || `Premium quality product.`}</p>
                        ...
                        {screen === "search" && (
                            <div className="w-full flex flex-col min-h-screen">
                                <div className="backdrop-blur-md border-b border-black/[0.06] w-full storefront-header p-6">
                                    <button onClick={() => changeScreen("home")} className="mb-4 text-gray-900" style={{ color: headerTextColor }}><ChevronLeft size={24} /></button>
                        ...
                        {screen === "cart" && (
                            <div className="w-full flex flex-col min-h-screen">
                                <div className="backdrop-blur-md border-b border-black/[0.06] w-full storefront-header p-6 flex items-center gap-4">
                                    <button onClick={() => changeScreen("home")} className="text-gray-900" style={{ color: headerTextColor }}><ChevronLeft size={24} /></button>
                                    <h2 className="text-xl font-black text-gray-900" style={{ color: headerTextColor }}>Your Bag</h2>
                                </div>
                                    ...
                                    {cartItemCount === 0 ? <p className="text-center text-gray-400 py-20 font-black uppercase tracking-widest">Bag is empty</p> : (
                                        <div className="space-y-4">
                                            ...
                                            <div className="pt-10 space-y-4">
                                                <div className="flex justify-between font-black text-gray-900" style={{ color: textColor }}><span>Total</span><span className="text-xl text-emerald-600">{s.currency}{totalPrice.toLocaleString()}</span></div>
                        ...
                        {screen === "community" && (
                            <div className="w-full flex flex-col min-h-screen">
                                <div className="backdrop-blur-md border-b border-black/[0.06] w-full storefront-header p-6 flex items-center gap-4">
                                    <button onClick={() => changeScreen("home")} className="text-gray-900" style={{ color: headerTextColor }}><ChevronLeft size={24} /></button>
                                    <h2 className="text-xl font-black text-gray-900" style={{ color: headerTextColor }}>Reviews</h2>
                                </div>
                                <div className="p-6">
                        ...
                        {screen === "success" && (
                            <div className="w-full flex flex-col items-center justify-center p-10 text-center min-h-screen bg-white" style={{ backgroundColor: bgColor }}>
                                <CheckCircle2 size={64} className="text-emerald-500 mb-6" />
                                <h2 className="text-3xl font-black text-gray-900 italic uppercase" style={{ color: textColor }}>Order Sent!</h2>
...
export function CustomerStorefrontPreview({
  ...
  const mockProducts: Product[] = [
    { id: 1, name: "Premium Item", description: "Handcrafted with care", price: 12500, image: "https://images.unsplash.com/photo-1523275335684-37898b6baf30", images: [], outOfStock: false, category: "Featured" },
    { id: 2, name: "Signature Piece", description: "Limited edition quality", price: 8900, image: "https://images.unsplash.com/photo-1542291026-7eec264c27ff", images: [], outOfStock: false, category: "New" },
    { id: 3, name: "Classic Collection", description: "Timeless design excellence", price: 15000, image: "https://images.unsplash.com/photo-1491553895911-0055eca6402d", images: [], outOfStock: false, category: "Popular" },
  ];
  ...
  return (
    <div className={wrapperClass}>
      {type === "hero" && (() => {
          ...
          return <Hero state={state} onShopClick={() => {}} />;
      })()}
      {type === "catalog" && (() => {
          const Catalog = CatalogTemplates[templateId] || CatalogTemplates["catalog-1"];
          return <Catalog state={state} products={mockProducts} onProductClick={() => {}} />;
      })()}
      {type === "about" && (() => {
          const About = AboutTemplates[templateId] || AboutTemplates["about-1"];
          return <About state={state} />;
      })()}
      {type === "footer" && (() => {
          const Footer = FooterTemplates[templateId] || FooterTemplates["footer-1"];
          return <Footer state={state} />;
      })()}
    </div>
  );
}